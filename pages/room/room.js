...
Page({
  ...
  onLoad: function (options) {
    // 019060510074311
    console.log(options)
    const index = options.isPush==='1'?1:0;
    this.data.roomID = options.roomID || 1234;
    this.data.userId = options.userId || account.users[index].userId;
    this.data.userSig = options.userSig || account.users[index].userToken;
    this.data.template = options.template ||'bigsmall';

    this.data.webrtcroomComponent = this.selectComponent('#webrtcroom');

    this.setData({
      template: options.template || 'bigsmall'
    });
